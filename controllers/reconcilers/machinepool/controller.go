...
func (r *Controller) ReconcileCreate(reconcileRequest request.Request) (ctrl.Result, error) {
	// type cast the request to a machine pool req
	req, ok := reconcileRequest.(*MachinePoolRequest)
	if !ok {
		return requeue.OnError(req, request.TypeConvertError(&MachinePoolRequest{}))
	}
